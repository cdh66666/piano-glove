...
丢音是**物理必然**（6 根手指，快曲必然按不过来），所以界面上直接写「这首曲子 N 个音按不了」，
...
- 两种分配方式：
  ...
  - **音高对应** —— 把本曲的**实际音域**均分给 6 个槽位，低音偏拇指侧、高音偏小指侧。
...
- 按住时长取音符时长的 80%（夹在 55~200ms），接近真人触键。**这条是"音符密≠难"的原因**：
  触键只占 55~200ms，6 根手指轮转足够，单声部旋律再快也压不垮；真正吃紧的是同一时刻要几根手指
...
| 级别 | 曲子 | 主要测什么 |
| --- | --- | --- |
| L1 入门 | 小星星 / 玛丽有只小羊羔 | 6 个音对 6 根手指，核对编号 |
...
演奏页下方那张图，上半张是**曲子本身**（横轴时间、纵轴音高），下半张是**手套的动作**（6 行 = 6 根手指），
...
- **灰色小块 = 手套来不及按、被丢掉的音**（只有 6 个自由度，保节奏优先）。它只画在上半张，
  因为它没有对应的动作。
